...
/* ── Generic GET ───────────────────────────────────────────── */
export async function tbGet(path, query = {}, forwardedToken = null) {
  const token = forwardedToken || await getTbToken();
...
/* ── Telemetry ─────────────────────────────────────────────── */
export async function getLatestTelemetry(deviceId, keys, token = null) {
...
export async function getTelemetryHistory(deviceId, key, hours = 1, limit = 1000, token = null) {
  const endTs = Date.now();
  const startTs = endTs - hours * 60 * 60 * 1000;
  // Use the forwarded browser token when available — avoids a server-side login
  // that Cloudflare may challenge for server-to-server requests.
  const t = token || await getTbToken();
  const qs = new URLSearchParams(
    Object.fromEntries(
      Object.entries({ keys: key, startTs, endTs, limit, orderBy: "ASC" })
        .map(([k, v]) => [k, String(v)])
    )
  ).toString();
  const url = `${TB_URL}/api/plugins/telemetry/DEVICE/${deviceId}/values/timeseries?${qs}`;
  const res = await fetch(url, { headers: { "X-Authorization": `Bearer ${t}` } });
  if (!res.ok) {
    const text = await res.text();
    throw new Error(`TB GET timeseries failed (${res.status}): ${text.slice(0, 120)}`);
  }
...
/* ── Attributes ────────────────────────────────────────────── */
export async function getDeviceAttributes(deviceId, keys = [], scope = "SERVER_SCOPE", token = null) {
  const raw = await tbGet(
 ...
 */
export function parseWaveformBatch(rawValue, batchTs, sampleIntervalMs = 10) {
  if (!rawValue) return [];
  try {
...
export async function getPatientInfo(deviceId, token = null) {
