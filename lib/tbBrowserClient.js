...
const GATEWAY_ID = process.env.NEXT_PUBLIC_TB_DEVICE_ID;
    ...
    nodeDevices.map(async (device) => {
      try {
        const attrs = await tbFetch(
          `/api/plugins/telemetry/DEVICE/${device.id}/values/attributes/SERVER_SCOPE?keys=active,lastActivityTime,lastConnectTime,lastDisconnectTime,patientName`,
          token
        );
        const map = {};
